...
<html>
<head>
  <title>Manage Students - EduSphere</title>
  <style>
    body {
      ...
      <tbody>
        @forelse($students as $student)
          <tr>
            ...
            <td>
  <form action="{{ route('admin.students.assign.class', $student->id) }}" method="POST">
    @csrf
    <select name="class_assigned" onchange="this.form.submit()">
      <option value="">Select Class</option>
      @for ($i = 1; $i <= 10; $i++)
        <option value="{{ $i }}" {{ $student->class_assigned == $i ? 'selected' : '' }}>
          Class {{ $i }}
        </option>
      @endfor
    </select>
...
<td class="actions">
  <a href="{{ route('edit.student', $student->id) }}" class="btn btn-edit">Edit</a>
